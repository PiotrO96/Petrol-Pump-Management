...
Imports System.Data
Public Class Supplier_withdraw_pump_bank_detail
    ...
    Dim i As Integer
    Dim ds As DataSet
    Private Sub Supplier_withdraw_pump_bank_detail_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    '    Try
    '        conn.Open()
    '        Dim str As String = "insert into suppwithdraw(suppacno,suppname,tdate,bname,actype,mofaction,chddno,tamt)values('" & ComboBox1.Text & "','" & TextBox1.Text & "','" & (DateTimePicker1.Value()).ToString() & "','" & cmbname.Text & "','" & ComboBox2.Text & "','" & cmbtrans.Text & "','" & TextBox2.Text & "','" & txtamt.Text & "')"
    ...
    '    Try
    '        Dim i As Integer = 0
    '        cmd = New OleDbCommand("select max(suppacno) from suppwithdraw ", conn)
    '        conn.Open()
    '        dr = cmd.ExecuteReader()
    '        If (dr.HasRows) Then
    '            While (dr.Read())
    ...
    Private Sub ComboBox1_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ComboBox1.SelectedIndexChanged
        Try
            ...
            If (dr.HasRows()) = True Then
                While (dr.Read())
                    ...
                    TextBox1.Text = dr("suppname").ToString
        ...
        End Try
        'Try
        '    cmd = New OleDbCommand("select * from suppaccount where orderno = " & ComboBox1.Text & "", conn)
        '    conn.Open()
        '    dr = cmd.ExecuteReader()
        '    If (dr.Read) = True Then
        '        'ComboBox1.Text = dr("suppacno").ToString
        '        ' TextBox1.Text = dr("suppname").ToString
    ...
    Private Sub btnedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnedit.Click
        cmd = New OleDbCommand("update suppwithdraw set suppname = '" & TextBox1.Text & "',tdate = '" & (DateTimePicker1.Value()).ToString() & "',bname= '" & cmbname.Text & "',actype='" & ComboBox2.Text & "',mofaction = '" & cmbtrans.Text & "',chddno = '" & TextBox2.Text & "',tamt = " & txtamt.Text & " ,pamt = " & TextBox3.Text & " where  suppacno = " & ComboBox1.Text & "", conn)
        conn.Open()
        cmd.ExecuteNonQuery()
    ...
    Private Sub Supplier_withdraw_pump_bank_detail_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.LostFocus
        'Try
        '    conn.Open()
        '    cmd = New OleDbCommand("select * from supplieraccount where orderno = " & ComboBox1.Text & "", conn)
        '    dr = cmd.ExecuteReader()
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        cmd = New OleDbCommand("update ttbank set tbnk = '" & TextBox5.Text & "' where bid = " & TextBox4.Text & "", conn)
        conn.Open()
        cmd.ExecuteNonQuery()
        MessageBox.Show("record successfully updated")
        conn.Close()
    ...
    Private Sub TextBox7_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox7.LostFocus
        Try
            conn.Open()
            cmd = New OleDbCommand("select * from ttbank where bid = " & TextBox4.Text & "", conn)
            dr = cmd.ExecuteReader()
            If (dr.HasRows()) = True Then
                While (dr.Read())
                    'DateTimePicker1.Value = dr("tdate").ToString
                    TextBox7.Text = dr("tbnk").ToString
                    'TextBox3.Text = dr("balamt").ToString
                End While
            End If
        Catch ex As Exception
    ...
    Private Sub Button6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button6.Click
        TextBox7.Text = Val(TextBox7.Text) - Val(txtamt.Text)
    End Sub
